...
@frappe.whitelist()
def preview_quick_order_items(items, company: str | None = None, delivery_date=None):
	prior_allocations = {}
	if company and delivery_date:
		from process_simplification.api.production import get_prior_finished_stock_allocations

		prior_allocations = get_prior_finished_stock_allocations(
			company, target_delivery_date=delivery_date
		)
	rows = []
	for index, raw_row in enumerate(_as_list(items), start=1):
		...
		try:
			...
			continue
		available_snapshot = max(normalize_qty(detail.get("available_to_reserve")), 0)
		prior_allocation = max(
			normalize_qty(prior_allocations.get((detail.get("item_code"), detail.get("warehouse")))),
			0,
		)
		coverage = min(max(available_snapshot - prior_allocation, 0), qty)
...
def _evaluate_quick_order(payload):
	...
	preview = preview_quick_order_items(data.get("items"), company, data.delivery_date)
