frappe.ui.form.on("Monthly Worker Wage Summary", {
	refresh(frm) {
		...
		if (frm.doc.docstatus === 0 && !frm.is_new() && monthEnded) {
			...
			frm.dashboard.add_comment(__("本工资月份已结束，请点击右上角「工资汇总 → 确认月度汇总」完成确认。"), "orange", true);
...
function openConfirmDialog(frm) {
	...
	const dialog = new frappe.ui.Dialog({
		title: __("确认月度工资汇总"),
		fields: [{ fieldtype: "HTML", options: `<p>${__("确认后，本月全部已通过报工将固定进入该汇总；Job Card 可以仍在继续报工。报工状态仍保持“已通过”。")}</p><p><strong>${frappe.utils.escape_html(frm.doc.employee_name || frm.doc.employee || "")} · ${frappe.utils.escape_html(frm.doc.wage_month || "")} · ${format_currency(frm.doc.total_amount || 0)}</strong></p>` }],
