frappe.pages["purchase-receipt-notice"].on_page_load = function (wrapper) {
	...
	async function load() {
		...
		const { message } = await frappe.call({ method: api + (name ? "get_notice" : "list_notices"), args: name ? { name } : {} });
		...
		if (!name) {
			const section = $('<section class="purchase-section"><div class="purchase-section-heading"><h2>最近到货记录</h2><span class="purchase-meta">本公司最近 300 条事件</span></div></section>').appendTo(root);
			if (!message.length) section.append('<div class="purchase-empty">暂无可查看的到货记录。</div>');
			else section.append(`<table class="purchase-table purchase-request-table"><thead><tr><th>收货单 / 供应商</th><th>发生时间</th><th>通知状态</th><th></th></tr></thead><tbody>${message.map((event) => `<tr><td><strong>${esc(event.supplier)}</strong><br>${esc(event.receipt)}</td><td data-label="发生时间">${esc(event.occurred_at)}</td><td data-label="通知状态">${esc(statusLabels[event.status] || event.status)}</td><td><a class="btn btn-default" href="/desk/purchase-receipt-notice?name=${encodeURIComponent(event.name)}">查看通知</a></td></tr>`).join("")}</tbody></table>`);
